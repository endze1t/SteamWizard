...
//                            $ContainerParent.css('height', iNewHeight + 'px');
    ...
    var loadCompleteInventory = function(callback) {
            var actualCode = '(' +
                function() {
                    g_ActiveInventory.LoadCompleteInventory().done(function() {
                        ...
                        g_ActiveInventory.SW_FullyLoaded = true;
                    })
                }
            + ')();';
        ...
        function repeat() {
            fetchGlobal('g_ActiveInventory', function (data) {
                if(data.SW_FullyLoaded) {
                   callback();
                } else if(--counter > 0) {
                    setTimeout(repeat, (50-counter) * 50);
                }
            }, function (input) {
                return {SW_FullyLoaded: input.g_ActiveInventory.SW_FullyLoaded};
            });
        };
    ...
    return {
        ...
        loadCompleteInventory: loadCompleteInventory
    };
});